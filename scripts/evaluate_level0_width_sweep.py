#!/usr/bin/env python3
"""Native paired evaluation for one point in the embedding_dim/level0_width
sweep (scripts/train_level0_width_sweep.py), versus current best
(outputs/higher_resolution_true_level0). Reuses the exact same sliding-
window/padding/export logic as every other experiment in this family.
"""
...
def main():
    ...
    ck = torch.load(OUT / f"checkpoints/best_level0_width_sweep_emb{emb}.pt", map_location="cpu", weights_only=False)
    decoder = TrueFullResolutionLevel0OneQueryMaskDecoder(emb, 4, level0_width=config["level0_width"])
    ...
    for r in records:
        ...
        item = {
            ...
            "sweep_raw_prediction_path": str(raw_path),
            "sweep_filtered_prediction_path": str(filtered_path),
            "sweep_probability_path": str(prob_path),
        }
        for name, p in (("baseline_raw", base["higher_res_raw_prediction_path"]), ("baseline_filtered", base["higher_res_filtered_prediction_path"]),
                        ("sweep_raw", raw_path), ("sweep_filtered", filtered_path)):
            item.update({f"{name}_{k}": v for k, v in full_metrics(np.asarray(nib.load(p).dataobj) > 0, gt, spacing).items()})
        comparison.append(item)
        print(f"{domain} {sid} baseline_dice={item['baseline_filtered_dice']:.5f} sweep_dice={item['sweep_filtered_dice']:.5f}", flush=True)
    ...
    for d in ("CAMRI", "Mouse"):
        ...
        for c in ("baseline_filtered", "sweep_filtered"):
            row = {"domain": d, "condition": c, "subjects": len(q), "embedding_dim": emb, "level0_width": config["level0_width"]}
