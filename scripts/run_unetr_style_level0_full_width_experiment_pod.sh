...
LOG=outputs/unetr_style_level0_full_width_decoder/pipeline_gpu_full_width.log
mkdir -p outputs/unetr_style_level0_full_width_decoder
echo "=== $(date) : GPU PIPELINE START ===" | tee -a "$LOG"

echo "=== $(date) : TRAIN ===" | tee -a "$LOG"
python3 scripts/train_unetr_style_level0_full_width.py 2>&1 | tee -a "$LOG"

echo "=== $(date) : EVALUATE ===" | tee -a "$LOG"
python3 scripts/evaluate_unetr_style_level0_full_width.py 2>&1 | tee -a "$LOG"

echo "=== $(date) : COMPARE / FIGURES ===" | tee -a "$LOG"
python3 scripts/compare_unetr_style_level0_full_width_results.py 2>&1 | tee -a "$LOG"

echo "=== $(date) : REPORT ===" | tee -a "$LOG"
python3 scripts/write_unetr_style_level0_full_width_report.py 2>&1 | tee -a "$LOG"
