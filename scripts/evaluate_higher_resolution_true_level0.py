#!/usr/bin/env python3
"""Native paired evaluation of the higher-resolution (192x128x160) TRUE
level0 decoder checkpoint, versus the valid 128x128x160 TRUE level0 baseline.
...
called completely unmodified. Two, and only two, things are monkeypatched
before calling them, both narrowly scoped to this experiment's sole
intervention (input/target resolution):

  1. FEATURE_NAMES includes "level0" (same monkeypatch every true-level0
     evaluation in this project uses).
  2. `preprocess` is swapped for `higher_resolution_preprocessing`'s
     spacing-aware variant, which internally calls the identical
     DefaultPreprocessor/resampling_fn as the baseline -- only the target
     spacing value differs (192x128x160 grid, axis0/native-Y refined).

Tiling, stride, whole-volume centered/symmetric padding, and overlap
averaging are therefore byte-identical to the baseline's own evaluation,
just operating on a different (finer) tile size, exactly as `sliding_window_logits`
already supports via its `tile_size` parameter.

Baseline comparator: outputs/true_full_resolution_level0_decoder's OWN
predictions (its "level0_*" columns), not corrected_label_retraining -- this
experiment's question is "does higher-resolution input help the TRUE level0
decoder," so the valid 128x128x160 TRUE level0 run is the correct baseline.
"""
...
BASELINE_OUT = ROOT / "outputs/true_full_resolution_level0_decoder"
STRUCT = np.ones((3, 3, 3), bool)

# The two sanctioned differences from the baseline's evaluation path, both
# scoped to this experiment's sole intervention (resolution):
...
def main():
    ...
    config = load_json(ROOT / "configs/higher_resolution_true_level0.yaml")
    ...
    ck = torch.load(OUT / "checkpoints/best_higher_resolution_true_level0.pt", map_location="cpu", weights_only=False)
    ...
    for r in records:
        ...
        item = {
            ...
            "baseline_raw_prediction_path": base["level0_raw_prediction_path"],
            "baseline_filtered_prediction_path": base["level0_filtered_prediction_path"],
            "baseline_probability_path": base["level0_probability_path"],
            "higher_res_raw_prediction_path": str(raw_path),
            "higher_res_filtered_prediction_path": str(filtered_path),
            "higher_res_probability_path": str(prob_path),
        }
        for name, p in (("baseline_raw", base["level0_raw_prediction_path"]), ("baseline_filtered", base["level0_filtered_prediction_path"]),
                        ("higher_res_raw", raw_path), ("higher_res_filtered", filtered_path)):
        ...
        source.append({
            ...
            "baseline_dice": item["higher_res_raw_dice"], "filtered_dice": item["higher_res_filtered_dice"],
            "dice_change": item["higher_res_filtered_dice"] - item["higher_res_raw_dice"],
        ...
        print(f"{domain} {sid} baseline_dice={item['baseline_filtered_dice']:.5f} higher_res_dice={item['higher_res_filtered_dice']:.5f}", flush=True)
    ...
    for d in ("CAMRI", "Mouse"):
        ...
        for c in ("baseline_raw", "baseline_filtered", "higher_res_raw", "higher_res_filtered"):
