...
LOG=outputs/paper_style_level0_decoder/pipeline_gpu_paper_style.log
mkdir -p outputs/paper_style_level0_decoder
echo "=== $(date) : GPU PIPELINE START ===" | tee -a "$LOG"

echo "=== $(date) : TRAIN ===" | tee -a "$LOG"
python3 scripts/train_paper_style_level0.py 2>&1 | tee -a "$LOG"

echo "=== $(date) : EVALUATE ===" | tee -a "$LOG"
python3 scripts/evaluate_paper_style_level0.py 2>&1 | tee -a "$LOG"

echo "=== $(date) : COMPARE / FIGURES ===" | tee -a "$LOG"
python3 scripts/compare_paper_style_level0_results.py 2>&1 | tee -a "$LOG"

echo "=== $(date) : REPORT ===" | tee -a "$LOG"
python3 scripts/write_paper_style_level0_report.py 2>&1 | tee -a "$LOG"
