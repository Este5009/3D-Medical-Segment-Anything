#!/usr/bin/env python3
"""Paired corrected-baseline versus TRUE level0 diagnostic evidence.

Adapted from scripts/compare_full_resolution_level0_results.py, pointed at
outputs/true_full_resolution_level0_decoder/. Analysis logic (contour
geometry, factor-N quantization, probability detail, paired figures) is
reused unchanged from diagnose_model_spatial_resolution.py and
analyze_boundary_error_diagnostics.py so the two level0 experiments remain
...
def geometry(records):
    ...
    for i, r in enumerate(records, 1):
        ...
        for condition, pred_key, raw_key, prob_key in (
            ...
            ("true_level0", "level0_filtered_prediction_path", "level0_raw_prediction_path", "level0_probability_path"),
        ):
...
def figure(r, role, path):
    ...
    rn = np.asarray(nib.load(r["level0_raw_prediction_path"]).dataobj) > 0
    pb, _ = align_probability_to_raw(np.asarray(nib.load(r["baseline_probability_path"]).dataobj, np.float32), rb)
    pn, _ = align_probability_to_raw(np.asarray(nib.load(r["level0_probability_path"]).dataobj, np.float32), rn)
    ...
    titles = ["MRI", "Expert mask", "Corrected baseline", "TRUE full-resolution level0", "Baseline FP/FN", "Level0 FP/FN",
              "Baseline probability", "Level0 probability", "Baseline boundary zoom", "Level0 boundary zoom",
              "Baseline error zoom", "Level0 error zoom"]
...
def main():
    ...
    metric = [r for r in rows(OUT / "native_metrics.csv") if r["condition"] in ("baseline_filtered", "level0_filtered")]
    ...
    for r in rec:
        paired.append({
            ...
            "baseline_dice": r["baseline_filtered_dice"], "level0_dice": r["level0_filtered_dice"],
            "dice_change": float(r["level0_filtered_dice"]) - float(r["baseline_filtered_dice"]),
            "baseline_hd95_mm": r["baseline_filtered_hd95_mm"], "level0_hd95_mm": r["level0_filtered_hd95_mm"],
            "baseline_assd_mm": r["baseline_filtered_assd_mm"], "level0_assd_mm": r["level0_filtered_assd_mm"],
        })
    ...
    for domain in ("CAMRI", "Mouse"):
        ...
        g = [x for x in geom if x["domain"] == domain and x["condition"] == "corrected_baseline"]
        pixel = max(g, key=lambda x: x["prediction_expert_axis_run_max_ratio"])["subject"]
        roles = {
            "largest_improvement": max(q, key=lambda x: float(x["level0_filtered_dice"]) - float(x["baseline_filtered_dice"])),
            "largest_regression": min(q, key=lambda x: float(x["level0_filtered_dice"]) - float(x["baseline_filtered_dice"])),
            "median_change": sorted(q, key=lambda x: float(x["level0_filtered_dice"]) - float(x["baseline_filtered_dice"]))[len(q) // 2],
            "previous_pixelated": next(x for x in q if x["subject"] == pixel),
            "previous_worst": min(q, key=lambda x: float(x["baseline_filtered_dice"])),
        }
        curved = "064" if domain == "CAMRI" else "POLYIC_20190510_mouse43__E9_P1"
        roles["high_curvature"] = next(x for x in q if x["subject"] == curved)
    ...
    ax.set(title="TRUE full-resolution level0 validation history", xlabel="Epoch", ylabel="Volumetric Dice", ylim=(.9, 1))
