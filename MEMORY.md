...
## Full-resolution level0 decoder ablation: implementation audit (2026-08-12)

- A read-only audit (`outputs/full_resolution_level0_decoder/implementation_audit.md`) found the level0 ablation above was confounded and its "does not help" conclusion was not trustworthy as a general finding. Two issues: (1) `configs/full_resolution_level0_decoder.yaml` carried an unused `corrected_baseline_checkpoint` field; the training script actually initialized shared decoder weights from `outputs/generalization_pilot/best_checkpoint.pt` (the pre-correction, pre-mixed-domain epoch-14 checkpoint), not the converged `best_corrected_labels.pt` -- confirmed by checkpoint metadata and a numeric check (max logit difference from the corrected baseline at initialization was 48-59 in logit space; the level0 model at init was instead bit-identical, max|Δ|=0.0, to the old pre-correction decoder). (2) The architecture's "native full-grid logits" were dominated by an unconditional `64×64×80 → 2x trilinear interpolation` pathway identical to the pre-existing decoder; only a small 8-channel residual, gated by a scalar initialized to exactly zero, was genuinely full-resolution. (3) Native evaluation used a locally reimplemented sliding-window tiling function with asymmetric (non-centered) edge padding, differing from the baseline's shared symmetric-padding implementation, for volumes with undersized axes (which several CAMRI/Mouse subjects have).
- Verdict: B (mostly valid, narrower hypothesis than claimed). The experiment validly compared "adding a small residual on top of the old architecture, both retrained from the same epoch-14 root" but did not validly test "does the corrected-label decoder benefit from being extended to genuine full-resolution decoding." Recommended repeating with the baseline-checkpoint initialization wired up and identical evaluation tiling; see the corrected re-run below.

## Genuine full-resolution level0 decoder ablation, corrected re-run (2026-08-13)

- `outputs/true_full_resolution_level0_decoder/` fixes both confounds above. `TrueFullResolutionLevel0OneQueryMaskDecoder` (`models/query_mask_decoder.py`, 182,081 params, `level0_width=16`) inherits levels4→1 byte-identically from `MultiScaleOneQueryMaskDecoder` and forms its mask logits with **one** dot product evaluated directly on a fused full-grid (`128×128×160`) feature field -- no half-resolution mask logits and no residual/skip term exist anywhere in the graph. Initialized from `outputs/corrected_label_retraining/checkpoints/best_corrected_labels.pt`; all 76/76 baseline decoder tensors transfer with zero mismatched values (verified on real CAMRI+Mouse samples pre-training, `pretraining_verification.json`). Target/logits both confirmed `[1,1,128,128,160]`, categorical `{0,1}`; no final logit upsampling occurs (bitwise-identical with/without an explicit `output_size` argument).
- Because there is no zero-gated residual trick, the new head starts far from the baseline (epoch-1 CAMRI Dice 0.853 vs. a 0.9728 safety floor) and needed several epochs to recover. Reusing the baseline's CAMRI safety rule as an immediate training-abort trigger (as literally written) aborted a first attempt after one epoch with an untrained checkpoint. The fix, used for the reported run: keep the identical numeric floor and "best balanced Dice" selection rule, but apply it as a per-epoch eligibility filter for checkpoint selection rather than a training-abort trigger; ineligible epochs still count toward the 20-epoch budget and do not consume early-stop patience. Epoch 9 was first eligible; epoch 20 (final, still improving) was selected, balanced validation Dice 0.977094.
- On the same untouched native test cohort, evaluated with the baseline's own unmodified sliding-window/symmetric-padding code (monkeypatched only to additionally request `level0` features): **Mouse improved** -- Dice 0.969536→0.972438, HD95 0.2009→0.1608 mm (-20.0%), ASSD 0.04214→0.03577 mm (-15.1%), total residual error 585,309→525,213 (-10.3%), boundary ≤1-voxel share 89.08%→91.62%, high-complexity error rate 39.700→35.709 per 100 surface voxels, factor-5 native-y excess 29.69→28.61 points, and 78/80 subjects individually improved. **CAMRI was neutral** -- Dice 0.987029→0.986922, HD95 unchanged at 0.1333 mm, total error +0.91%, 3/6 subjects improved and 3/6 regressed (n=6, near the metric's measurement floor already).
- One disclosed trade-off: predictions became more decisive (fewer uncertain boundary voxels), and while total Mouse FP fell 21.6%, the FP/FN voxels that remain wrong are more often confidently wrong (Mouse boundary confident-incorrect FP 27.48%→33.57%). Qualitative figures show the full-resolution field capturing shape detail (lobe separations, concavities) the half-resolution baseline smooths over, at the cost of mild internal speckle in some slices; no figure shows gross failure.
- Scientific conclusion: **the prior "level0 does not help" finding was an artifact of the confounded implementation, not evidence about full-resolution decoding itself.** With correct initialization and a genuine (non-residual) full-resolution mask head, level0 decoding measurably helps Mouse boundary geometry and accuracy and is neutral (not harmful) on CAMRI. This architecture path should not be abandoned. Suggested next step: a residual-failure/attribution pass on these predictions, and a check of whether the Mouse-specific gain reflects genuine cross-domain transfer or simply more capacity being available to fit Mouse's share of `balanced_epoch_order` training.

## Boundary-stage-localization and higher-resolution-encoder diagnostics (2026-08-13)

- Two fast, read-only, no-training diagnostics (`outputs/boundary_stage_localization/`, `outputs/higher_resolution_encoder_diagnostic/`) traced 3 representative Mouse subjects through every pipeline stage. Finding: native→model preprocessing alone (no network involved) destroys 16-32% of the true expert boundary's direction-change detail, because the model grid already covers 1.6-3.6 native voxels per model voxel before the frozen encoder ever runs; the continuous probability field's smoothness is essentially unchanged between model space and native space, so resampling/thresholding do not add further loss beyond that ceiling.
- The encoder diagnostic confirmed the frozen RS2-Net encoder (a windowed-attention Swin transformer with no size-locked parameters) accepts larger input with the checkpoint loading strict, and that increasing resolution specifically on the worst-loss axis (model axis0 → native Y) measurably improved both the geometric ceiling and the encoder's own level0 boundary-gradient-contrast on 3/3 subjects, at a near-linear (~1.5x time for 1.5x voxels) cost. This directly motivated the controlled retraining below.

## Higher-resolution (192×128×160) TRUE level0 decoder (2026-08-13)

- `outputs/higher_resolution_true_level0/` changed exactly one variable relative to the valid `outputs/true_full_resolution_level0_decoder` baseline: model-space input/target resolution, refined from `128×128×160` to `192×128×160` on model axis0 only (→ native Y, the dominant loss axis; spacing 0.25→0.16667 mm, FOV preserved exactly at 32.0 mm). Architecture, loss, optimizer, schedule, seed, split, augmentation, threshold, and filter were all unchanged.
- Because `TrueFullResolutionLevel0OneQueryMaskDecoder`'s parameters are all channel-dimension-only (no spatial D/H/W dependence), initializing from `outputs/true_full_resolution_level0_decoder/checkpoints/best_true_level0_decoder.pt` transferred **100% of weights** (102/102 tensors, 182,081/182,081 parameters, zero new parameters) — verified, not assumed. Unlike the original true-level0 experiment, this run never needed the CAMRI-safety-eligibility-filter machinery (`camri_safety_stop: false`, no ineligible epochs; epoch 1 CAMRI Dice was already 0.976).
- Training ran the full 20-epoch budget; selected epoch 17, balanced validation Dice 0.980386 (vs. the 128-resolution baseline's own final 0.977094).
- On the same untouched native test cohort, evaluated with the baseline's own unmodified sliding-window code (only the preprocessing spacing was swapped): **both domains improved on every subject, with zero regressions.** CAMRI (6/6 improved): Dice 0.986922→0.988834, ASSD 0.02049→0.01782 mm (-13.0%), total error -13.4%. Mouse (80/80 improved): Dice 0.972438→0.979537 (+0.0071), HD95 0.1608→0.1151 mm (-28.4%), ASSD 0.03577→0.02601 mm (-27.3%), total error 525,213→392,748 (-25.2%).
- Boundary/pixelation, the primary endpoint, improved more than any prior intervention in this project: contour direction-change ratio (closer to 1.0 = more articulated) rose 0.698→0.845 (Mouse) and 0.829→0.921 (CAMRI); factor-5 native-y grid-lock excess more than halved in both domains (Mouse 28.61→12.69 points, CAMRI 18.08→8.08 points); high-complexity error rate fell 18.5% (Mouse, 35.709→29.10 per 100 surface voxels); boundary ≤1-voxel share rose to 94.6% (Mouse) and 94.3% (CAMRI). Unlike the original 128-resolution level0 change, predictions did not become more confidently wrong — Mouse boundary confident-incorrect FP fell 33.57%→26.42%.
- Efficiency: parameters unchanged (182,081; resolution-only change). Training epoch cost rose 249→581 s (2.33x, roughly matching the 1.5x voxel increase plus decoder overhead); peak memory rose only 10,236→11,252 MiB. Native inference rose 1.76x for Mouse (9.46→16.68 s/subject) but 6.6x for CAMRI (9.93→65.15 s/subject, likely because CAMRI's larger native extent now needs more sliding-window tiles at the finer grid) — a disclosed, domain-specific cost affecting only 6 subjects, not a general property of the change.
- Scientific conclusion: **higher-resolution input solves a meaningful part of the remaining boundary problem** (verdict A). This confirms the boundary-stage-localization diagnostic's identification of preprocessing/input-resolution as the primary bottleneck, rather than the decoder or encoder. Recommended as the new baseline resolution for this decoder family. Natural next step (not attempted here, to keep this a single-variable change): also refining model axis2 (→ native X, the second loss-bearing axis) to test whether the gain compounds further.
