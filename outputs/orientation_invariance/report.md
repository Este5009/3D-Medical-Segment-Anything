...
2. **Correction (2026-09-16): the trainable decoder trunk does not just inherit that drift — it partially compounds it.** Each canvas level fuses the upsampled coarser canvas with that level's own skip connection straight from the encoder; both are already drifted, by different amounts, and a plain conv block has no way to reconcile two independently-misaligned inputs. `canvas_level2` (~0.41) is nearly as high as the single worst stage in the whole pipeline (`encoder_level0`, ~0.43) -- well above every other encoder level. Only the final step, the dot product against the (nearly rotation-invariant) query, pulls the number back down to ~0.17 at the output. An earlier version of this line claimed the trunk "neither removes nor compounds" the drift; the per-stage numbers above do not support that.
