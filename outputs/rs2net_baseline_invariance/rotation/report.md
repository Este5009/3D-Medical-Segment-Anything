# RS2-Net baseline rotation-invariance vs. this project's model
...
- This project's model (frozen RS2-Net encoder + this project's own decoder, NO rotation augmentation): mean self-consistency Dice **0.9764** (`outputs/orientation_invariance/brain/summary_by_mask.csv`).

If the baseline number above is meaningfully higher than this project's own 0.9764, that is direct evidence the paper's rotation-augmented training measurably helps -- and a concrete case for adding the same kind of augmentation to this project's own decoder training. If the two are close, the gap this project measured is coming from somewhere other than missing rotation augmentation (e.g. the decoder's skip-fusion compounding, or the lesion task specifically, which this baseline cannot speak to at all).
