...
2. **Correction (2026-09-16): the trainable decoder trunk does not just inherit that drift — it compounds it.** Each canvas level fuses the upsampled coarser canvas with that level's own skip connection straight from the encoder; both are already drifted, by different amounts, and a plain conv block has no way to reconcile two independently-misaligned inputs. `canvas_level2` (~0.34) is in fact the single highest-drift stage in the entire pipeline -- higher than any encoder level on its own (the worst, `encoder_level2`, is ~0.30). Only the final step, the dot product against the (nearly translation-invariant) query, pulls the number back down to ~0.22 at the output. An earlier version of this line claimed the trunk "neither removes nor compounds" the drift; the per-stage numbers above do not support that.
